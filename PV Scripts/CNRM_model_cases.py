...
def load_datasets_for_case(case, year_start, year_end):
    ...
    # Load datasets for the 2080-2100 period (assuming CNRM data)
    vas_file_path = f'/work/nai5/storage/interpolated_vas_CNRM_{adjusted_start_year}-{adjusted_end_year}_grid.nc'
    uas_file_path = f'/work/nai5/storage/interpolated_uas_CNRM_{adjusted_start_year}-{adjusted_end_year}_grid.nc'
    tas_file_path = f'/datacommons/gpeos/nai5/var_data/Variables/tas/GFDL/interpolated/interpolated_tas_GFDL_{adjusted_start_year}-{adjusted_end_year}_grid.nc'
    rsds_file_path = f'/work/nai5/storage/interpolated_rsds_CNRM_{adjusted_start_year}-{adjusted_end_year}_grid.nc'
    ...
    if case == 1:
        ...
        rsds_file_path_2020_2040 = f'/work/nai5/storage/interpolated_rsds_CNRM_{adjusted_start_year_2020_2040}-{adjusted_end_year_2020_2040}_grid.nc'
        rsds_ds_2020_2040 = xr.open_dataset(rsds_file_path_2020_2040)

        print(f"Shape of rsds_ds['rsds'] before replacement: {rsds_ds['rsds'].shape}")
        print(f"Shape of rsds_ds_2020_2040['rsds']: {rsds_ds_2020_2040['rsds'].shape}")

        # Check if the variable needs to be transposed
        if rsds_ds_2020_2040['rsds'].dims != rsds_ds['rsds'].dims:
            rsds_ds_2020_2040['rsds'] = rsds_ds_2020_2040['rsds'].transpose(*rsds_ds['rsds'].dims)
        ...
        adjusted_start_year_2020_2040 = adjusted_start_year - 60
        adjusted_end_year_2020_2040 = adjusted_end_year - 60
        tas_file_path_2020_2040 = f'/datacommons/gpeos/nai5/var_data/Variables/tas/GFDL/interpolated/interpolated_tas_GFDL_{adjusted_start_year_2020_2040}-{adjusted_end_year_2020_2040}_grid.nc'
        tas_ds_2020_2040 = xr.open_dataset(tas_file_path_2020_2040)

        print(f"Shape of tas_ds['tas'] before replacement: {tas_ds['tas'].shape}")
        print(f"Shape of tas_ds_2020_2040['tas']: {tas_ds_2020_2040['tas'].shape}")

        # Check if the variable needs to be transposed
        if tas_ds_2020_2040['tas'].dims != tas_ds['tas'].dims:
            tas_ds_2020_2040['tas'] = tas_ds_2020_2040['tas'].transpose(*tas_ds['tas'].dims)
        ...
        adjusted_start_year_2020_2040 = adjusted_start_year - 60
        adjusted_end_year_2020_2040 = adjusted_end_year - 60
        vas_file_path_2020_2040 = f'/work/nai5/storage/interpolated_vas_CNRM_{adjusted_start_year_2020_2040}-{adjusted_end_year_2020_2040}_grid.nc'
        uas_file_path_2020_2040 = f'/work/nai5/storage/interpolated_uas_CNRM_{adjusted_start_year_2020_2040}-{adjusted_end_year_2020_2040}_grid.nc'
        vas_ds_2020_2040 = xr.open_dataset(vas_file_path_2020_2040)
        uas_ds_2020_2040 = xr.open_dataset(uas_file_path_2020_2040)

        print(f"Shape of vas_ds['vas'] before replacement: {vas_ds['vas'].shape}")
        print(f"Shape of vas_ds_2020_2040['vas']: {vas_ds_2020_2040['vas'].shape}")
        print(f"Shape of uas_ds['uas'] before replacement: {uas_ds['uas'].shape}")
        print(f"Shape of uas_ds_2020_2040['uas']: {uas_ds_2020_2040['uas'].shape}")

        # Check if the variables need to be transposed
        if vas_ds_2020_2040['vas'].dims != vas_ds['vas'].dims:
            vas_ds_2020_2040['vas'] = vas_ds_2020_2040['vas'].transpose(*vas_ds['vas'].dims)
        if uas_ds_2020_2040['uas'].dims != uas_ds['uas'].dims:
            uas_ds_2020_2040['uas'] = uas_ds_2020_2040['uas'].transpose(*uas_ds['uas'].dims)
...
def main():
    for year in range(2089, 2100, 2):
        year_start = year
        year_end = year + 1

        for case in range(1, 4):  # Loop through cases 1, 2, and 3
            #output_file_path = f'output/CNRM/CNRM_daily_energy_output_case{case}_{year_start}-{year_end}.nc'
            output_file_path = f'/work/nai5/storage/CNRM_output/CNRM_daily_energy_output_case{case}_{year_start}-{year_end}.nc'

            # Load datasets based on the case
            ...
            common_time = rsds_ds['time'].values
            if len(vas_ds['time'].values) == len(uas_ds['time'].values) == len(tas_ds['time'].values) == len(common_time):
            ...
            print(f"Results saved successfully for case {case}, {year_start}-{year_end}.")
